...
During this process the organisms are sorted and selected based on their fitness relative to others in the population. The selection process can be tweaked by setting the `reproductionPercentage` property on `EvolutionManager`. By setting this to any `CGFloat` value between (non-inclusive) 0.0 and 1.0, you determine the number of organisms that are selected for the breeding pool. 

>***Example:*** If you have a population of 100 organisms, and you set the `reproductionPercentage` to `0.2`, this will result in 20 organisms being chosen for breeding. If this value is not set, the default value of `0.33` is used, resulting in approximately 1/3 of the population being selected for breeding.

Tangential to the reproduction rate is the `elitismPercentage` property of the `EvolutionManager`. To simulate some amount of [***elitism***](http://en.wikipedia.org/wiki/Genetic_algorithm#Elitism), you're able to customize the percentage of organisms from the current generation that will survive and continue into the next. This property can be set to any `CGFloat` value between 0.0 (inclusive) to 1.0 (non-inclusive), and indicates the percentage of the total population `n` that survives through to generation `n + 1`.
